...
defineModule('System.ProjectExport', ({ Utils, DocsManager, UI, Config }) => {
  ...
  function exportCurrentProjectSource() {
    ...
    try {
      ...
      if (!scriptId) {
        throw new new Error('Could not get current script ID. Is the Apps Script API enabled?');
      }

      // 3. الحصول على ملف السكريبت نفسه من Drive.
      // هذا الملف يمثل المشروع بأكمله في Google Drive.
      const scriptFile = DriveApp.getFileById(scriptId);
      // قراءة محتوى الملف كـ Blob ثم تحويله إلى نص.
      const scriptContent = scriptFile.getBlob().getDataAsString();
