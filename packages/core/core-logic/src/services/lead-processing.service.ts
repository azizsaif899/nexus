// Mock OdooClient for build
interface OdooClient {
  createLead: (data: any) => Promise<any>;
  updateLead: (id: string, data: any) => Promise<any>;
  findPartnerByEmail: (email: string) => Promise<any>;
  findPartnerByPhone: (phone: string) => Promise<any>;
  addContactNote: (contactId: number, note: string) => Promise<void>;
}
const OdooClient = {} as any;
