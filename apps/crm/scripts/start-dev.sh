...
# Configuration
PORT=5173
WORKSPACE_ROOT="/path/to/nexus"        # Update this path - Nx workspace root
...
navigate_to_project() {
    print_step "Step 2: Navigating to workspace root..."
    
    if [ -d "$WORKSPACE_ROOT" ]; then
        cd "$WORKSPACE_ROOT" || exit 1
        print_success "Current directory: $WORKSPACE_ROOT"
        
        # Check if app exists
        if [ ! -d "$WORKSPACE_ROOT/apps/$APP_NAME" ]; then
            print_error "App directory not found: apps/$APP_NAME"
            print_info "Please update the APP_NAME variable in this script"
            read -p "Press Enter to exit..."
            exit 1
        fi
        print_success "App found: apps/$APP_NAME"
    else
        print_error "Workspace root not found: $WORKSPACE_ROOT"
        print_info "Please update the WORKSPACE_ROOT variable in this script"
        read -p "Press Enter to exit..."
        exit 1
    fi
}
...
start_dev_server() {
    ...
    print_info "Starting Nx dev server for $APP_NAME on port $PORT..."
