  ...
  _createTraversalVisitors(fileContent, issues) {
    ...
    return {
      ...
      VariableDeclarator(path) {
        // اكتشاف المتغيرات العالمية المحتملة (بدون let/let/const)
        ...
        if (path.node.id.name && !path.scope.hasBinding(path.node.id.name) && path.scope.parent === null) {
          // المتغير معرّف في الجذر وليس له `let`, `let`, `const`
          ...
          issues.push(self._createIssue({
            ...
            codeSnippet: self._getCodeSnippet(fileContent, path.node.loc),
            fixable: false,
            suggestedFix: `// CONSIDER: Ensure '${path.node.id.name}' is declared with 'const', 'let', or 'let' to prevent global scope pollution.`
          }));
        }
      },
    };
