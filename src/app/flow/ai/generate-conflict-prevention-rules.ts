...
const GenerateConflictPreventionRulesInputSchema = z.object({
  projectName: z.string().describe('The name of the project for which to generate conflict prevention rules.'),
  projectType: z.string().describe('The type of the project (e.g., application, library).'),
  projectScope: z.string().describe('The scope of the project (e.g., shared, nexux).'),
});
export type GenerateConflictPreventionRulesInput = z.infer<typeof GenerateConflictPreventionRulesInputSchema>;

const GenerateConflictPreventionRulesOutputSchema = z.object({
  eslintRules: z.string().describe('Generated ESLint rules to enforce module boundaries and naming conventions.'),
  typescriptPathMappings: z.string().describe('Generated TypeScript path mappings to ensure proper module resolution.'),
  environmentVariablePrefix: z.string().describe('Generated environment variable prefix to avoid naming conflicts with other environment variables.'),
});
export type GenerateConflictPreventionRulesOutput = z.infer<typeof GenerateConflictPreventionRulesOutputSchema>;

export async function generateConflictPreventionRules(input: GenerateConflictPreventionRulesInput): Promise<GenerateConflictPreventionRulesOutput> {
