...
class HealthCheckV2 {
  ...
  async run() {
    console.log('🔍 بدء فحص صحة النظام v2.0...\n');
  ...
  async checkServices() {
    ...
    for (const service of this.services) {
      try {
        ...
        console.log(`   ${status === 'healthy' ? '✅' : '❌'} ${service.name}: ${status}`);
      } catch (error) {
        results.push({
          ...service,
          status: 'unhealthy',
        ...
        console.log(`   ❌ ${service.name}: غير متاح`);
      }
    }
  ...
  async checkSidebarAgents() {
    console.log('🎨 فحص الوكلاء الذكيين...');
    const results = [];
    ...
    for (const agent of this.agents) {
      ...
      console.log(`   ${exists ? '✅' : '❌'} ${agent} Agent: ${exists ? 'نشط' : 'غير موجود'}`);
    }
    ...
    for (const mode of this.modes) {
      ...
      console.log(`   ${exists ? '✅' : '❌'} ${mode} Mode: ${exists ? 'نشط' : 'غير موجود'}`);
    }
  ...
  async checkSearchSystems() {
    ...
    for (const system of this.searchSystems) {
      ...
      console.log(`   ${exists ? '✅' : '❌'} ${system}: ${exists ? 'متاح' : 'غير موجود'}`);
    }
  ...
  async checkPythonServices() {
    ...
    try {
      ...
      console.log(`   ✅ Python Runtime: ${pythonVersion.trim()}`);
    } catch (error) {
      results.push({
        name: 'python_runtime',
        status: 'missing',
      ...
      console.log('   ❌ Python Runtime: غير متاح');
    }
    ...
    try {
      ...
      console.log(`   ${isHealthy ? '✅' : '❌'} Gemini Backend: ${isHealthy ? 'يعمل' : 'متوقف'}`);
    } catch (error) {
      results.push({
        name: 'gemini_backend',
        status: 'stopped',
      ...
      console.log('   ❌ Gemini Backend: متوقف');
    }
  ...
  async checkHybridIntegration() {
    console.log('🔄 فحص التكامل الهجين...');
    const results = [];
    ...
    if (fs.existsSync(apiPath)) {
      ...
      console.log(`   ${(hasResearchEndpoint && hasSidebarEndpoint) ? '✅' : '⚠️'} API Endpoints: ${(hasResearchEndpoint && hasSidebarEndpoint) ? 'مكونة' : 'ناقصة'}`);
    }
    ...
    if (fs.existsSync(mainPath)) {
      ...
      console.log(`   ${hasCors ? '✅' : '❌'} CORS Config: ${hasCors ? 'مكون' : 'غير مكون'}`);
    }
  ...
  displaySummary(results) {
    ...
    console.log(`🌐 الخدمات: ${summary.healthy_services || 0}/${summary.total_services || 0} تعمل`);
    console.log(`🎨 الوكلاء: ${summary.active_agents || 0}/${summary.total_agents || 0} نشط`);
    console.log(`🔍 أنظمة البحث: ${summary.available_search_systems || 0}/${summary.total_search_systems || 0} متاح`);
    
    const overallStatus = this.calculateOverallStatus(results);
    const statusIcon = overallStatus === 'healthy' ? '✅' : overallStatus === 'degraded' ? '⚠️' : '❌';
    console.log(`📈 الحالة العامة: ${statusIcon} ${overallStatus}`);
