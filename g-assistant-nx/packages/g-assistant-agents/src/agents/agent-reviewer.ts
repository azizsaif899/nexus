...
export class AgentReviewer {
  ...
  async analyzeCodeReview(sheetId: string, severity: string = 'all') {
    ...
    const result = await this.dataConnect.query(query, { sheetId, severity });
  ...
  private calculateQualityScore(data: any) {
    ...
    return Math.max(Math.min(score, 100), 0).toFixed(2);
  }
  ...
  private calculateIssuePriority(issue: any) {
    ...
    return (issue.count * weight).toString();
  }

  private generateCodeInsights(data: any) {
    const insights = [];
    const qualityScore = parseFloat(this.calculateQualityScore(data));
