import { createDataConnect } from '../client';
import { getCRMService } from '../services/crm-service';

export class IntegrationTests {
  private dataConnect = createDataConnect({
  ...
  private async testDataConnectClient() {
    ...
    try {
      const isHealthy = await this.dataConnect.healthCheck();
      if (isHealthy) {
    ...
    try {
      const result = await this.dataConnect.query(`query { health { status } }`);
      if (result) {
