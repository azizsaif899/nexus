...
class NexusChatPro {
    ...
    private async loadSystemContext() {
        try {
            ...
            console.warn('Some system data unavailable:', error);
        }
    }
    ...
    private switchPersona(personaId: string) {
        ...
        // Enhanced system instruction with context
        const enhancedInstruction = `${instruction}

SYSTEM CONTEXT:
- CRM Clients: ${this.crmClients.length} active clients
- Project Status: ${this.systemContext.project?.status || 'Unknown'}
- Available Data: CRM, Google Sheets, Project Files
- You can access and query this data to help users.

When users ask about clients, projects, or data, use the available context.`;
    ...
    private async handleSubmit(e: Event) {
        ...
        try {
            ...
            const contextualInput = await this.enhanceUserInput(userInput);
            
            const result = await this.chat.sendMessage(contextualInput);
    ...
    private async enhanceUserInput(userInput: string): Promise<string> {
        const lowerInput = userInput.toLowerCase();
        let contextData = '';
        
        // Check for CRM queries
        if (lowerInput.includes('عميل') || lowerInput.includes('client') || lowerInput.includes('crm')) {
            try {
                const response = await fetch('/api/crm/search?q=' + encodeURIComponent(userInput));
                const data = await response.json();
                contextData += `\nCRM Data: ${JSON.stringify(data.slice(0, 5))}`;
            } catch (error) {
        ...
        if (lowerInput.includes('مشروع') || lowerInput.includes('project') || lowerInput.includes('code')) {
            try {
                const response = await fetch('/api/project/files');
                const data = await response.json();
                contextData += `\nProject Files: ${JSON.stringify(data.slice(0, 10))}`;
            } catch (error) {
                contextData += '\nProject Files: Not available';
            }
        }
        
        // Check for sheets queries
        if (lowerInput.includes('شيت') || lowerInput.includes('sheet') || lowerInput.includes('data')) {
            try {
                const response = await fetch('/api/sheets/query?q=' + encodeURIComponent(userInput));
                const data = await response.json();
                contextData += `\nSheets Data: ${JSON.stringify(data)}`;
            } catch (error) {
                contextData += '\nSheets Data: Not available';
            }
        }
        
        return userInput + contextData;
    }
    ...
    private async handleFileUpload(file: File) {
        ...
        try {
            ...
            this.addMessage(`تم تحليل الملف: ${file.name}\n\nالملخص: ${analysis.summary}`, 'ai');
        } catch (error) {
            this.addMessage(`خطأ في تحليل الملف: ${error.message}`, 'ai');
        }
    }
}
