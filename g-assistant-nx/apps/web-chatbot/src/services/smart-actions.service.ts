...
export class SmartActionsService {
  ...
  private async convertLeadToOpportunity(params: any): Promise<any> {
    try {
      ...
      return {
        success: false,
        message: `خطأ في تحويل العميل المحتمل: ${error.message}`
      };
    }
  }
  ...
  private async addNoteToOpportunity(params: any): Promise<any> {
    try {
      ...
      return {
        success: false,
        message: `خطأ في إضافة الملاحظة: ${error.message}`
      };
    }
  }
  ...
  private async createQuotation(params: any): Promise<any> {
    try {
      ...
      return {
        success: false,
        message: `خطأ في إنشاء عرض السعر: ${error.message}`
      };
    }
  }
  ...
  private async scheduleActivity(params: any): Promise<any> {
    try {
      ...
      return {
        success: false,
        message: `خطأ في جدولة النشاط: ${error.message}`
      };
    }
  }
  ...
  private async updateLeadStage(params: any): Promise<any> {
    try {
      ...
      const stageId = stageMapping[params.stage_name];
      if (!stageId) {
        return {
          success: false,
      ...
      return {
        success: false,
        message: `خطأ في تحديث المرحلة: ${error.message}`
      };
    }
  }
  ...
  private async getCustomerInsights(params: any): Promise<any> {
    try {
      ...
      const insights = {
        ...
        total_expected_revenue: opportunities.reduce((sum, opp) => sum + (opp.expected_revenue || 0), 0),
        avg_probability: opportunities.length > 0 
          ? opportunities.reduce((sum, opp) => sum + (opp.probability || 0), 0) / opportunities.length 
          : 0,
        latest_opportunities: opportunities.slice(0, 3).map(opp => ({
          name: opp.name,
      ...
      return {
        success: false,
        message: `خطأ في جلب رؤى العميل: ${error.message}`
      };
    }
  }
  ...
  async executeAction(actionName: string, parameters: any): Promise<any> {
    ...
    try {
      ...
      return {
        success: false,
        message: `خطأ في تنفيذ الإجراء: ${error.message}`
      };
    }
  }
}